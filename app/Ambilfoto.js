...
export default class Ambilfoto extends React.Component {
    _isMounted = false;
    constructor(props) {
        super(props);
        this.state = {
    ...
    this.props.navigation.goBack('Pengiriman', { 
        divisi: this.state.divisi, 
        user: this.state.id,
        url: BASE_URL + '/web/index.php?r=android/pengiriman&divisi=' + this.state.divisi + '&user=' + this.state.id + '&lat=' + this.state.lat + '&lon=' + this.state.lon
    });
    ...
    _getLocationAsync = async () => {
        ...
        if (inet === true && status === 'granted' && gps === true) {
            const { pengiriman } = this.props.route.params;
            await Location.watchPositionAsync({ accuracy: Location.Accuracy.Highest }, ( async point => {
                ...
                console.log({ gps: gps, pengiriman: this.state.pengiriman, location:location })
            })).catch(err => {
                return
            })
        } else {
    ...
    simpanimg = async () => {
        ...
        console.log({
            pengiriman: this.state.pengiriman,
            id: this.state.id,
            lat: this.state.lat,
            lon: this.state.lon,
            fileType: fileType,
        ...
        if (this.state.lat !== '' && this.state.lon !== '') {
            ...
            formData.append('pengiriman', this.state.pengiriman);
            ...
            try {
                const url = BASE_URL + '/web/index.php?r=apiandroid/ambilfoto';
